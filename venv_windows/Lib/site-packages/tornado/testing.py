...
class _TestMethodWrapper(object):
    ...
    method yields it must use a decorator to consume the generators),
...
class AsyncTestCase(unittest.TestCase):
    ...
    def __init__(self, methodName='runTest'):
        ...
        # the generators.  Replace the test method with a wrapper that will
        # make sure it's not an undecorated generators.
...
def gen_test(func=None, timeout=None):
    ...
    def wrap(f):
        # Stack up several decorators to allow us to access the generators
        # object itself.  In the innermost wrapper, we capture the generators
        ...
        def post_coroutine(self, *args, **kwargs):
            try:
                ...
                # If the underlying generators is still running, we can throw the
                # exception back into it so the stack trace is replaced by the
                # point where the test is stopped. The only reason the generators

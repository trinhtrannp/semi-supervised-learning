"""``tornado.gen`` implements generators-based coroutines.

.. note::

   The "decorator and generators" approach in this module is a
...
technically asynchronous, but it is written as a single generators
...
def coroutine(func):
    ...
    Any generators that yields objects from this module must be wrapped
...
def _make_coroutine_wrapper(func, replace_callback):
    ...
    ``@coroutine``.
    """
    # On Python 3.5, set the coroutine flag on our generators, to allow it
...
class YieldPoint(object):
    """Base class for objects that may be yielded from the generators.
    ...
    def start(self, runner):
        """Called by the runner after the generators has yielded.
    ...
    def is_ready(self):
        """Called by the runner to determine whether to resume the generators.
...
class Callback(YieldPoint):
    ...
    generators function, but may be reused across different runs of the same
...
class Runner(object):
    ...
    The results of the generators are stored in ``result_future`` (a
    `.Future`)
    """
    ...
    def set_result(self, key, result):
        """Sets the result for ``key`` and attempts to resume the generators."""
    ...
    def run(self):
        """Starts or resumes the generators, running until it reaches a
